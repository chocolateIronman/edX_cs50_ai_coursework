...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in Nim.available_actions(list(state)):
            maximum_q = max(self.get_q_value(state, action), maximum_q)
        return maximum_q
    ...
    def choose_action(self, state, epsilon=True):
        ...
        """
        if epsilon == False or (epsilon == True and self.epsilon < random.random()):
            best_value = -1
            best_action = None
            for action in Nim.available_actions(state):
                ...
                elif self.get_q_value(state, action) > best_value:
                    best_value = self.get_q_value(state, action)
                    best_action = action
            return best_action
        else:
